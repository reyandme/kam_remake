...
//    Offer, Demand: TKMDeliveryEntityID;
    OfferID: Integer;
    OfferWareType: TKMWareType;
    DemandID: Integer;
    DemandWareType: TKMWareType;
...
type
  ...
  private
    ...
    procedure CloseDemand(aWareType: TKMWareType; aID: Integer);
    procedure CloseOffer(aWareType: TKMWareType; aID: Integer);
    function ValidDelivery(oWT, dWT: TKMWareType; iO, iD: Integer; aIgnoreOffer: Boolean = False): Boolean;
    function SerfCanDoDelivery(oWT: TKMWareType; iO, iD: Integer; aSerf: TKMUnitSerf): Boolean;
...
//    function GetUnitsCntOnPath(aNodeList: TKMPointList): Integer;
    ...
    function GetDemandCount(aWareType: TKMWareType): Integer;
    function GetDemand(aWareType: TKMWareType; aDemandID: Integer): TKMDeliveryDemand;
    function GetOfferCount(aWareType: TKMWareType): Integer;
    function GetOffer(aWareType: TKMWareType; aOfferID: Integer): TKMDeliveryOffer;
  ...
  public
    ...
    property Demand[aWareType: TKMWareType; aDemandID: Integer]: TKMDeliveryDemand read GetDemand;
    property DemandCount[aWareType: TKMWareType]: Integer read GetDemandCount;

    property Offer[aWareType: TKMWareType; aOfferID: Integer]: TKMDeliveryOffer read GetOffer;
    property OfferCount[aWareType: TKMWareType]: Integer read GetOfferCount;
...
procedure TKMHandLogistics.UpdateState(aTick: Cardinal);
  ...
  function AnySerfCanDoDelivery(oWT: TKMWareType; iO,iD: Integer): Boolean;
  var
    I: Integer;
  begin
    Result := False;
    for I := 0 to fSerfCount - 1 do
      if fSerfs[I].IsIdle and fQueue.SerfCanDoDelivery(oWT, iO, iD, fSerfs[I]) then
        Exit(True);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        for oWT in oWTset do
          for iD := 0 to fQueue.fDemandCount[dWT] - 1 do
            if fQueue.fDemand[dWT, iD].IsUsed
              ...
              for iO := 0 to fQueue.fOfferCount[oWT] - 1 do
                if fQueue.fOffer[oWT, iO].IsUsed
                  and fQueue.ValidDelivery(oWT, dWT, iO, iD)
                  and AnySerfCanDoDelivery(oWT, iO, iD) then //Only choose this delivery if at least one of the serfs can do it
                begin
      ...
      if bid <> nil then
      begin
        offerPos := fQueue.fOffer[bid.OfferWareType, bid.OfferID].Loc_House.PointBelowEntrance;
        fQueue.fBestBidCandidates.Clear;
        fQueue.fBestBidCandidates.EnlargeTo(fSerfCount);
        serfBid := nil;
        for K := 0 to fSerfCount - 1 do
          if fSerfs[K].IsIdle then
            if fQueue.SerfCanDoDelivery(bid.OfferWareType, bid.OfferID, bid.DemandID, fSerfs[K]) then
            begin
        ...
        if serfBid <> nil then
        begin
          fQueue.AssignDelivery(bid.OfferWareType, bid.DemandWareType, bid.OfferID, bid.DemandID, serfBid.Serf);
          serfBid.Free;
        end;
        bid.Free;
      end;
...
begin
  ...
  I := 0;
  while (I < fOfferCount[aWare]) and fOffer[aWare,I].IsUsed do
    Inc(I);
...
begin
  if (fQueue[aIQ].DemandID <> DELIVERY_NO_ID) and (fQueue[aIQ].DemandWareType <> wtNone) then
    Result := not fDemand[fQueue[aIQ].DemandWareType, fQueue[aIQ].DemandId].IsDeleted //Delivery could be cancelled because of Demand marked as Deleted
  else
...
begin
  ...
  for I := 0 to fQueueCount - 1 do
  begin
    if (fQueue[I].DemandWareType = aWareType)
      and (fQueue[I].JobStatus = jsTaken) then
    begin
      iD := fQueue[I].DemandId;
      if (fDemand[aWareType,iD].Loc_House = aHouse)
        and not fDemand[aWareType,iD].IsDeleted
        and (fDemand[aWareType,iD].BeingPerformed > 0) then
        Inc(Result);
    end;
  end;
end;
...
function TKMDeliveries.GetDemand(aWareType: TKMWareType; aDemandID: Integer): TKMDeliveryDemand;
begin
  Result := fDemand[aWareType, aDemandID];
end;


function TKMDeliveries.GetDemandCount(aWareType: TKMWareType): Integer;
begin
  Result := fDemandCount[aWareType];
end;


function TKMDeliveries.GetOffer(aWareType: TKMWareType; aOfferID: Integer): TKMDeliveryOffer;
begin
  Result := fOffer[aWareType, aOfferID];
end;


function TKMDeliveries.GetOfferCount(aWareType: TKMWareType): Integer;
begin
  Result := fOfferCount[aWareType];
end;
...
begin
  ...
  begin
    I := 0;
    while (I < fDemandCount[aWare]) and fDemand[aWare, I].IsUsed do
      Inc(I);
...
function TKMDeliveries.SerfCanDoDelivery(oWT: TKMWareType; iO,iD: Integer; aSerf: TKMUnitSerf): Boolean;
var
  LocA, LocB: TKMPoint;
begin
...
begin
  Result := ValidDelivery(oWT, dWT, iO, iD) and SerfCanDoDelivery(oWT, iO, iD, aSerf);
end;
...
begin
  ...
  Result := fQueue[aQueueID].OfferWareType;

  if Result in WARES_VALID then Exit;

  Result := fQueue[aQueueID].DemandWareType;
end;
...
var
  ...
  demandTaken: array[Low(fDemandCount)..High(fDemandCount)] of array of Boolean; //Each demand can only be taken once in our measurements
  dWT, oWT: TKMWareType;
  oWTset: TKMWareTypeSet;
begin
  {$IFDEF PERFLOG}
      ...
      for oWT in oWTset do
        ...
        begin
          ...
          for iD := 0 to fDemandCount - 1 do
            if not demandTaken[dWT,iD] and ValidDelivery(oWT,dWT,iO,iD) then
            begin
...
//    for iO := 0 to fOfferCount[WT] - 1 do
...
begin
  ...
  try
    ...
    oWT := fQueue[aDeliveryID].OfferWareType;
    oldD := fQueue[aDeliveryID].DemandID;
    oldDWT := fQueue[aDeliveryID].DemandWareType;
    ...
    begin
      ...
      fQueue[aDeliveryID].DemandID := bestDWT;
...
procedure TKMDeliveries.DeliveryFindBestDemand(aSerf: TKMUnitSerf; aDeliveryId: Integer; aWare: TKMWareType;
  ...
  begin
    ...
    //which can produce demand^ed ware (stone or wood)
    if Result
      ...
      begin
        ...
        if H.IsComplete
          ...
          and H.CanHaveWareType(demand^.Ware) then
  ...
  begin
    ...
    for dWT := Low(fDemandCount) to High(fDemandCount) do
      for iD := 0 to fDemandCount[dWT] - 1 do
        ...
        begin
          bid := TKMDeliveryBid.Create(fDemand[iD].Importance, aSerf, wtNone, dWT, 0, iD);
    ...
    begin
      ...
      for iD := 0 to fDemandCount[wtAll] - 1 do
        ...
        begin
          bid := TKMDeliveryBid.Create(fDemand[iD].Importance, aSerf, wtNone, wtAll, 0, iD);
          if TryCalculateBidBasic(dckFast, aSerf.Position, 1, htNone, aSerf.Owner, bid, nil, allowOffroad) then
          begin
            fBestBidCandidates.Push(bid);
            bestImportance := bid.Importance;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        fQueue[aDeliveryId].DemandWareType := bestDWT;
...
begin
  ...
  try
    ...
    if bid <> nil then
    begin
      AssignDelivery(bid.OfferWareType, bid.DemandWareType, bid.OfferID, bid.DemandID, aSerf);
      bid.Free;
      Result := True;
    end else
      ...
      begin
        ...
        for iQ := 0 to fQueueCount - 1 do
          if (fQueue[iQ].JobStatus = jsTaken)
            ...
            and (fQueue[iQ].OfferWareType <> wtNone)
            and (fQueue[iQ].DemandWareType <> wtNone)
            and (fOffer[fQueue[iQ].OfferWareType, fQueue[iQ].OfferID].Loc_House = aHouse)
            and (fDemand[fQueue[iQ].DemandWareType, fQueue[iQ].DemandID].Importance >= bestImportance)
            and (TKMTaskDeliver(fQueue[iQ].Serf.Task).DeliverStage = dsToFromHouse) then // Serf can walk in this house
          begin
            bid := TKMDeliveryBid.Create(fDemand[fQueue[iQ].DemandID].Importance, aSerf,
                                         fQueue[iQ].OfferWareType, fQueue[iQ].DemandWareType,
...
begin
  ...
  fQueue[I].DemandWareType := dWT;
  fQueue[I].OfferID := iO;
  fQueue[I].OfferWareType := oWT;
...
begin
  ...
  dWT := fQueue[iQ].DemandWareType;

  fQueue[iQ].DemandID := DELIVERY_NO_ID; //We don't need it any more
  fQueue[iQ].DemandWareType := wtNone;
...
begin
  ...
  fQueue[aID].OfferWareType := wtNone;
  fQueue[aID].DemandWareType := wtNone;
...
procedure TKMDeliveries.CloseDemand(aWareType: TKMWareType; aID: Integer);
begin
  Assert(fDemand[aWareType,aID].BeingPerformed = 0);

  if fDemand[aWareType,aID].NotifyLocHouseOnClose and (fDemand[aWareType,aID].Loc_House <> nil) then
    fDemand[aWareType,aID].Loc_House.DecResourceDelivery(aWareType);

  fDemand[aWareType,aID].NotifyLocHouseOnClose := False;
  fDemand[aWareType,aID].Ware := wtNone;
  fDemand[aWareType,aID].DemandType := dtOnce;
  fDemand[aWareType,aID].Importance := Low(TKMDemandImportance);
  gHands.CleanUpHousePointer(fDemand[aWareType,aID].Loc_House);
  gHands.CleanUpUnitPointer(fDemand[aWareType,aID].Loc_Unit);
  fDemand[aWareType,aID].IsDeleted := False;

  fDemand[aWareType,aID].Cleanup;
end;


procedure TKMDeliveries.CloseOffer(aWareType: TKMWareType; aID: Integer);
begin
  Assert(fOffer[aWareType,aID].BeingPerformed = 0);
  fOffer[aWareType,aID].IsDeleted := false;
  fOffer[aWareType,aID].Ware := wtNone;
  fOffer[aWareType,aID].Count := 0;
  gHands.CleanUpHousePointer(fOffer[aWareType,aID].Loc_House);

  fOffer[aWareType,aID].Cleanup;
end;
...
begin
  ...
  for I := 0 to fQueueCount - 1 do
    ...
    begin
      ...
      SaveStream.Write(OfferWarType, SizeOf(OfferWarType));
      SaveStream.Write(DemandWarType, SizeOf(DemandWarType));
...
begin
  ...
  begin
    ...
    SetLength(fOffer, fOfferCount[WT]);
  ...
  begin
    ...
    SetLength(fDemand, fDemandCount[WT]);
    for I := 0 to fDemandCount[WT] - 1 do
      with fDemand[WT,I] do
      begin
        LoadStream.Read(Ware, SizeOf(Ware));
