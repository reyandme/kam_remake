...
begin
  ...
  gTerrain.Land^[Y, X].BaseLayer.Rotation := KaMRandom(4{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.BrushTerrainTile'{$ENDIF}); //Random direction for all plain tiles
    ...
    case aObjType of
      otTrees:        begin
                        case gCursor.MapEdForestAge of
                          0:  treeAge := TREE_AGE[KaMRandom(Length(TREE_AGE){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF})];
                          1:  treeAge := TREE_AGE[KaMRandom(Length(TREE_AGE) - 1{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF})];
                          else
      ...
      otAllButTrees:  begin
                        objRandom := KaMRandom(High(RandomObjects[aTerrainKind]){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomObjects[aTerrainKind, objRandom];
                      end;
      otFlowers:      if (tpWolf in gTerrain.Land^[aY, aX].Passability) then begin
                        objRandom := KaMRandom(High(RandomFlowers){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
      ...
      otMushrooms:    if   (aTerrainKind in [tkGrass..tkPaleGrass])
                      ...
                      begin
                        objRandom := KaMRandom(High(RandomMushrooms){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomMushrooms[objRandom];
                      end;
      otStumps:       if   (aTerrainKind in [tkGrass..tkGrassSand3])
                        or (aTerrainKind in [tkGrassDirt..tkDirt]) then
                      begin
                        objRandom := KaMRandom(High(RandomStumps){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomStumps[objRandom];
                      end;
      otDeadTrees:    if   (aTerrainKind in [tkGrass..tkGrassSand3])
                        or (aTerrainKind in [tkGrassDirt..tkDirt]) then
                      begin
                        objRandom := KaMRandom(High(RandomDeadTrees){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomDeadTrees[objRandom];
                      end;
      otStones:       if (tpMakeRoads in gTerrain.Land^[aY, aX].Passability) then
                      begin
                        objRandom := KaMRandom(High(RandomStones){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomStones[objRandom];
                      end;
      otBushes:       if aTerrainKind in [tkGrassSand3..tkSand] then
                      begin
                        objRandom := KaMRandom(High(RandomBush){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomBush[objRandom];
                      end;
      otCactus:       if aTerrainKind in [tkGrassSand3..tkSand] then
                      begin
                        objRandom := KaMRandom(High(RandomCactus){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
                        Result := RandomCactus[objRandom];
                      end;
      otRuins:        if (tpMakeRoads in gTerrain.Land^[aY, aX].Passability) then
                      begin
                        objRandom := KaMRandom(High(RandomRuins){$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.PickRandomObject'{$ENDIF});
...
begin
  ...
  mostTKI := mostTKIs[KaMRandom(I{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.TryGetVertexEffectiveTerKind'{$ENDIF})];
...
begin
  ...
  if aTerrainKind in [tkStone..tkIronMount, tkCoal..tkIron] then
    //Equal chance
    Result := RandomTiling[aTerrainKind, KaMRandom(RandomTiling[aTerrainKind, 0]{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.GetRandomTile'{$ENDIF}) + 1]
  else
  if KaMRandom(6{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.GetRandomTile 2'{$ENDIF}) = 1 then
    //Chance reduced to 1/6
    Result := RandomTiling[aTerrainKind, KaMRandom(RandomTiling[aTerrainKind, 0]{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.GetRandomTile 3'{$ENDIF}) + 1];
end;
...
//    or (LandTerKind[pY+1,pX+1].TerKind <> tkCustom) then
  ...
  begin
    ...
    rot := KaMRandom(4{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.RebuildTile'{$ENDIF}); //random direction for all plain tiles
  end;
...
begin
  ...
  gTerrain.UpdateWalkConnect([wcWalk, wcFish, wcWork], rect, True);
end;
...
begin
  ...
  begin
    ...
    for OT := Low(TKMTerrainObjectType) to High(TKMTerrainObjectType) do
      ...
      begin
        key := KaMRandom(400 div gCursor.MapEdObjectsDensity{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.BrushObjects'{$ENDIF});
        if key < 2 then
        begin
...
begin
      ...
      begin
        ...
        begin
          ...
          if tmp <> 0 then // Tmp = 0 outside of hsCircle area
            ...
            tmp := KaMRandom(2{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.ApplyHeight'{$ENDIF})*3 - 2;
        end
        else
          tmp := 0;
       //end Unequalize
  ...
  gTerrain.UpdateLighting(R);
  gTerrain.UpdatePassability(R);
  gTerrain.UpdateWalkConnect([wcWalk, wcFish, wcWork], R, True);
end;
    ...
    cmTiles:        if (ssLeft in gCursor.SState) then
                    begin
                      if gCursor.MapEdDir in [0..3] then //Defined direction
                        ...
                        EditTile(gCursor.Cell, gCursor.Tag1, KaMRandom(4{$IFDEF DBG_RNG_SPY}, 'TKMTerrainPainter.UpdateStateIdle'{$ENDIF}));
                    end;
