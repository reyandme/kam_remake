...
function TKMUnitActionGoInOut.FindBestExit(const aLoc: TKMPoint): TKMBestExit;
  ...
  begin
    ...
    if (aL and aR) then
    begin
      if KaMRandom(2{$IFDEF DBG_RNG_SPY}, 'TKMUnitActionGoInOut.FindBestExit.ChooseBestExit'{$ENDIF}) = 0 then
        Result := beLeft
      else
        Result := beRight;
    end
...
begin
  ...
  //the unit makes a noticeable "jump". This needs to be updated after starting because we don't know about an
